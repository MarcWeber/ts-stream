...
describe("Transform", () => {
		...
		it(
			...
			clockwise(async () => {
				...
				expect(dest).to.deep.equal([
					...
					//         2ms timeout)
                                        [5], // timeout waiting for 6
					[6], // timeout waiting for 7
					[7], // timeout waiting for 8
					[8, 9],
				]);
			})
		);
	...
	describe("batch_marc()", () => {
		...
		it("batches values", async () => {
			const batched = s.transform(batcher_marc(2));
			const toWrite = [1, 2, 3];
			const writes = [
				...toWrite.map((n) => track(s.write(n))),
				track(s.end()),
		...
		it("with `minBatchSize`, forms batch if write not pending", async () => {
			...
			const batched = source.transform(batcher_marc(3, { minBatchSize: 2 }));
			const dest = await resolveBatchToAsyncValues(batched);
		...
		it(
			"applies backpressure to a `maxBatchSize` write from a `minBatchSize` write",
			clockwise(async () => {
				const batched = s.transform(batcher_marc(2, { minBatchSize: 1 }));
				const toWrite = [1, 2, 3];
				const writes = [
					...toWrite.map((n) => track(s.write(n))),
					track(s.end()),
		...
		it(
			...
			clockwise(async () => {
				...
				const batched = s.transform(batcher_marc(5, { minBatchSize: 2 }));
				const resolvers: (() => void)[] = [];
				batched.forEach(async (batch) => {
					const deferred = defer();
					resolvers.push(deferred.resolve);
			...
			for (const testCase of [
				...
				it(
					...
					clockwise(async () => {
						...
						const batched = s.transform(batcher_marc(1));
		...
		it(
			...
			clockwise(async () => {
				...
				const batched = pipeWithDelay(source).transform(
					batcher_marc(3, { flushTimeout: 50 })
				);
				...
				expect(dest).to.deep.equal([
					...
					[6], //    Processed alone because stream ends
					[7], //    Processed alone because stream ends
				]);
			})
		);
		...
		it(
			...
			clockwise(async () => {
				...
				const batched = slowEndingSource.transform(batcher_marc(1));
				const mres = track(batched.result());
				readInto(batched, batchResults);
		...
		it("aborts from source to sink", async () => {
			const sink = s.transform(batcher_marc(1)).map(identity);
			const ab = track(sink.aborted());
			s.abort(abortError);
			await settle([ab.promise]);
			expect(ab.reason).to.equal(abortError);
